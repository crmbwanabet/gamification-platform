...
export default function TutorialModal({ tutorialKey, onClose, closing }) {
  ...
  return (
    <div className={`fixed inset-0 bg-black/90 flex items-center justify-center z-[80] p-4 ${closing ? 'anim-backdrop-close' : 'anim-fade-in'}`} onClick={onClose}>
      <div className={`bg-gradient-to-b from-[#0a1520] to-[#030810] rounded-3xl max-w-lg w-full overflow-hidden border-0 shadow-2xl shadow-purple-900/50 max-h-[90vh] overflow-y-auto ${closing ? 'anim-modal-close' : 'anim-scale-in'}`} onClick={(e) => e.stopPropagation()}>
        {/* Header Image */}
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3 mb-6">
            <button
              ...
              disabled={step === 0}
              className={`flex-1 py-3 rounded-xl font-bold transition-all ${step === 0 ? 'bg-gray-800/40 border border-gray-600/20 opacity-50' : 'bg-black/40 hover:bg-purple-900/30 border border-white/10'}`}
            >
              ← Back
            </button>
            {step < tutorial.steps.length - 1 ? (
              <button
                ...
                className="flex-1 py-3 bg-gradient-to-r from-purple-500 to-purple-700 hover:from-purple-600 hover:to-purple-800 rounded-xl font-bold"
              >
                Next →
              </button>
            ) : (
