...
- **Store (LIVE pipeline, gated on the JWT key)**: items come from remote config (CRM dashboard stocks them). Purchases are server-backed: `POST /api/purchase` (identity from the verified bwanabet JWT — NEVER client-supplied; atomic stock via `purchase_item` RPC; rate-limited) → Telegram admin-group message with a ✅ credit button (`lib/telegram/`) → `/api/telegram/webhook` (secret-verified) or the CRM Fulfillment queue credits/rejects. Rejections refund exactly-once on the player's next session (`refundedPurchaseIds`). TRUST POSTURE (user decision 2026-07-17): production runs with `SSO_ALLOW_UNVERIFIED=true` and no JWT signing key — tokens are forgeable, ACCEPTED because purchases credit the NAMED player's bwanabet account and withdrawals are locked to the registered phone number (an attacker can't route value to themselves). Residual: coin balances aren't verified server-side, so admins MUST sanity-check the player's coin history (Players tab) before crediting — the Fulfillment UI says so. If the bwanabet JWT key ever becomes available, set it and drop the override for defense in depth
