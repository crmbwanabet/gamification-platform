...
- **`GamificationManager`** module + tab in `index.html` (vanilla JS, existing Tailwind + Chart.js + Lucide), visible to admin/developer only. Sub-tab layout (approved wireframe A):
  ...
  3. **Games & Economy** — 7 game cards (enabled toggle, dailyPlays stepper) + global economy panel (extraPlayCost editable; **maxWin displayed read-only** — the games' pay tables read the hardcoded `GAME_ECONOMY.MAX_WIN` at module scope and are scaled to it, so a remote maxWin override does nothing until the games are rewired; discovered in Plan A code review, deferred). Saves write `platform_config`; UI notes "live next session".
